...
class EnhancedCORSMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        if (isinstance(allow_origins, str) and allow_origins == "*") or \
           (isinstance(allow_origins, list) and "*" in allow_origins):
            ...
            print("✅ CORS: Using wildcard origin (*), credentials disabled")
        else:
    ...
    def _create_preflight_response(self, request: Request):
        ...
        # ✅ Return proper Response instead of JSONResponse
...
class JWTAuthMiddleware(BaseHTTPMiddleware):
    """Middleware untuk JWT Authentication dengan blacklist check"""
    ...
    def __init__(
        ...
        self.public_paths = public_paths or [
            ...
            "/static/.*", "/storage/.*", "/favicon.ico",
            # Hapus public paths yang ada di router `/public`
            # Mereka sudah di-handle di luar `api_router`
        ]
        self.exclude_paths = exclude_paths or ["/metrics", "/_ah/.*"]

    async def dispatch(self, request: Request, call_next):
        start_time = time.time()

        # ✅ FIX: Lewatkan request OPTIONS (preflight) ke middleware selanjutnya
        # Karena CORSMiddleware dipasang paling awal di main.py, ini
        # memastikan preflight request ditangani dengan benar.
        if request.method == "OPTIONS":
            return await call_next(request)

        # ✅ Lewatkan path yang dikecualikan (misal: health check)
        if self._is_path_excluded(request.url.path):
            response = await call_next(request)
            return response
        
        # ✅ Periksa apakah path-nya public
        if self._is_public_path(request.url.path):
            response = await call_next(request)
            self._add_process_time_header(response, start_time)
            return response

        # ✅ Jika bukan public path, verifikasi JWT
        ...
        # ... (sisa kode verifikasi token di sini) ...
        ...
        try:
            ...
            try:
                ...
                if is_blacklisted:
                    return JSONResponse(
                        status_code=status.HTTP_401_UNAUTHORIZED,
                        content={"success": False, "message": "Token telah di-revoke. Silakan login kembali.", "error": "token_revoked"},
                        headers={"WWW-Authenticate": "Bearer"}
                    )
            finally:
                db.close()
            ...
            if not exp or time.time() > exp:
                return JSONResponse(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    content={"success": False, "message": "Token telah kedaluwarsa", "error": "token_expired"},
                    headers={"WWW-Authenticate": "Bearer"}
                )
            ...
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
                content={"success": False, "message": f"Token tidak valid: {str(e)}", "error": "invalid_token"},
                headers={"WWW-Authenticate": "Bearer"}
            )
        except Exception as e:
            return JSONResponse(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                content={"success": False, "message": f"Error autentikasi: {str(e)}", "error": "server_error"}
            )
...
def setup_middleware(
    ...
    rate_limit: int = 60,
    cors_origins: Union[List[str], str, None] = None,
    ...
    default_public_paths = [
        "/", "/docs", "/redoc", "/openapi.json", "/health",
        "/api/auth/login", "/api/auth/register", "/api/auth/refresh",
        "/static/.*", "/storage/.*", "/favicon.ico", "/predict/.*", "/public/.*" # ✅ Tambahkan /public/.*
    ]

    # ✅ 1. PASANG CORS MIDDLEWARE PALING AWAL
    app.add_middleware(
        EnhancedCORSMiddleware,
        allow_origins=cors_origins or ["*"], # Gunakan konfigurasi yang diberikan
        allow_credentials=cors_allow_credentials
    )
    print("✅ CORS configured with EnhancedCORSMiddleware")
